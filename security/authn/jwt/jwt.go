...
func (a *Authenticator) Authenticate(ctx context.Context) (*engine.AuthClaims, error) {
	tokenString, err := engine.AuthFromMD(ctx, engine.BearerWord)
	if err != nil {
...
func (a *Authenticator) CreateIdentityWithContext(ctx context.Context, claims engine.AuthClaims) (context.Context, error) {
	strToken, err := a.CreateIdentity(claims)
	if err != nil {
		return ctx, err
	}

	ctx = engine.MDWithAuth(ctx, engine.BearerWord, strToken)
