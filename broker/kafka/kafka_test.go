	...
	return func(ctx context.Context, event broker.Event) error {
		...
		case []byte:
			if err := json.Unmarshal(t, &msg); err != nil {
				log.GetLogger().Error(nil, "json Unmarshal failed: ", err.Error())
				return err
			}
		case string:
		...
		default:
			log.GetLogger().Error(nil, "unknown type Unmarshal failed: ", fmt.Sprintf("%v", t))
...
func createTracerProvider(serviceName string) broker.Option {
	...
	if err != nil {
		LogErrorf("failed to create tracer provider: %v", err)
		return nil
	}
...
func Test_Publish_WithTracer(t *testing.T) {
	...
	b := NewBroker(
		broker.WithAddress(testBrokers),
		broker.WithCodec("json"),
		createTracerProvider("tracer_tester"),
	)
...
func Test_Publish_WithGlobalTracer(t *testing.T) {
	...
	tp, err := otlp.New(
		otlp.WithEndpoint("localhost:4317"),
		otlp.WithServiceName("global_tracer_tester"),
		otlp.WithInsecure(true),
	)
	if err != nil {
